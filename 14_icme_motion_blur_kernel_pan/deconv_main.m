function [I_pra, kernel] = deconv_main(B, ori_B, lambda_kernel, lambda_smooth,...
...
% %   kernel = zeros(k1,k2);
  ...
  %every level solver
      ...
      for i = 1:innerItr
          ...
          r = edge_map(Bp,window_size);
          r= exp(-r.^(0.8));
          structure = structure_adaptive_map( I,lambda_texture*r, 100,0.95);
          %structure =  my_L0Smoothing(I, lambda_texture, 2);
          %structure  = I;
...
% %           subplot(222);imshow(structure,[]);title('structure')
          ...
          MAP = reconsEdge3(I_x,I_y);
          %figure(111); imshow(MAP,[]);
          if size(I_x) ==size(Bx)
              [N1,N2]=size(I_x);
...
%           %% for test
          ...
          %%
%           dirname = 'interim_itr\TV';
%           mat_outname=sprintf('%s/pyramid%d_inner%d.mat',dirname,itr,i);
%           eval(sprintf('save  %s kernel I ' ,mat_outname));
      end
      %% debug;
      if (itr>1)
...
%     ori_B = rgb2ycbcr(double(ori_B) / 255);
%   end  
% fprintf('image deblurring...\n');
% for c= 1:size(ori_B,3)
%     %% will be replaced by our fast algorithm 
%     I_pra(:,:,c) = deconv_ansio_L1(ori_B(:,:,c),kernel,0.003,200);
%     %% using the edge preserveing model
%     %% will be replaced by our fast algorithm 
%      %I_pra(:,:,c) = deconvSps_adaptive_L1(ori_B(:,:,c),kernel,0.003,200,structure);
% end
I_pra = I;
