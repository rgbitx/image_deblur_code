...
% fsize = 5;% gaussian filter
% if psf_size < fsize
%     fsize = psf_size;
% end
% gfilter = ones(fsize)/(fsize^2);% gaussian filter
% denoised = conv2(padarray(latent,[floor(fsize/2),floor(fsize/2)],'replicate'),gfilter,'valid');
% denoised = bilateral_filter(latent,2.0,0.5,'replicate', 2);
denoised = latent;
%%
% derivative filters
dx = [-1 1; 0 0];
